<?php 
		...
		if( isset($_POST['email']) && !empty($_POST['email'])){
			$pattern = " /^[_a-z0-9-]+(\.[_a-z0-9-]+)*@[a-z0-9-]+(\.[a-z0-9-]+)*(\.[a-z]{2,4})$/ ";
			    if(preg_match($pattern, $_POST['email'])){
					...
					$message_mail = "Invalid Email";
				}
		}else{
			$message_email = '<b class="text-danger">Please Enter Email</b>';
		}

	//Password Condition
	if(isset($_POST['password']) && !empty($_POST['password'])){
		if(strlen($_POST['password']) < 6){
			$message_pass = '<b class="text-danger">Password must be 6 characters</b>';
		}else{
			$password = $_POST['password'];
		}
	}else{
		$message_pass = '<b class="text-danger">Please Enter Password</b>';
	}
	...
	if( (isset($email) && !empty($email)) && (isset($password) && !empty($password)) ){
		...
		if($row <= 0){
			$message_found = '<div class="text-danger text-center"><strong>OOP\'s!</strong> Email or Password is incorrect.</div>';
		}//num rows if
	}//isset if
}//signif ends
